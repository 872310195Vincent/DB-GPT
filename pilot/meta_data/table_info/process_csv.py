...
grouped_data = merged_data.groupby(['database_name', 'table_name', 'table_comment'])
...
for (db_name, table_name, table_comment), group_data in grouped_data:
    column_names = ''
    column_types = ''
    column_comments = ''
    ...
    all_comment = f"{db_name}.{table_name} {table_comment} {column_comment}"
    df_table_col_clean.append([db_name, table_name, column_names[:-1],column_types[-1],column_comments[-1],table_comment,all_comment,ddl_sql])
df_table_col_clean = pd.DataFrame(df_table_col_clean,
                                  columns=['database_name', 'table_name', 'column_name', 'column_type', 'column_comment', 
                                           'table_comment', 'all_comment', 'ddl_sql'])
